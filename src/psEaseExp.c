...
void onBangMsg(t_psEaseExp *_x) {
    ...
    {
        default:
        case 0: outlet_float(_x->out, easeInExponent4Phasor(_x->input, _x->curve));
                break;
        case 1: outlet_float(_x->out, easeOutExponent4Phasor(_x->input, _x->curve));
                break;
        case 2: outlet_float(_x->out, easeInOutExponent4Phasor(_x->input, _x->curve));
                break;
        case 3: outlet_float(_x->out, easeOutInExponent4Phasor(_x->input, _x->curve));
                break;
    }
}
